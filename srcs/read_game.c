...
/*   Updated: 2017/09/08 19:34:04 by dmulish          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_piece(t_s *s, int *j)
{
	if (*j == 0)
	{
		s->x_piece = ft_atoi(ft_strsplit(s->buf, ' ')[1]);
		s->y_piece = ft_atoi(ft_strsplit(s->buf, ' ')[2]);
...
void	read_map(t_s *s)
{
	...
	while (get_next_line(1, &(s->buf)) > 0 && ++i)
	{
...
void	read_game(t_s *s)
{
	...
	while (get_next_line(1, &(s->buf)) > 0)
	{
		...
		{
			s->x_map = ft_atoi(ft_strsplit(s->buf, ' ')[1]);
			s->y_map = ft_atoi(ft_strsplit(s->buf, ' ')[2]);
			free(s->buf);
			break ;
		}
	}
